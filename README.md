...
```
tanzu apps workload create weatherforecast-fsharp -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/weatherforecast-fsharp-source \
  --type web
```
...
```
tanzu apps workload get weatherforecast-fsharp
```
